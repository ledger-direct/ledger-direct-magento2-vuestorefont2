  ...
  <div>
    ...
    <div class="summary">
      <div class="summary__group">
        ...
        <VsfPaymentProvider @status="onPaymentMethodChanged" />
...
import { useLedgerDirectStore } from '~/modules/ledger-direct/stores/ledgerDirectStore';
...
export default defineComponent({
  ...
  setup() {
    ...
    const ledgerDirectStore = useLedgerDirectStore();
    const onPaymentMethodChanged = (paymentMethodName: string) => {
      ledgerDirectStore.setPaymentMethodName(paymentMethodName);
    ...
    const processOrder = async () => {
      if (ledgerDirectStore.paymentMethodName === 'xrp_payment') {
        order.value = await make();
        setCart(null);
        app.$vsf.$magento.config.state.removeCartId();
        await load();
        await removeItem('checkout');
        const ledgerDirectRoute = app.localeRoute({
          name: 'ledger-direct-payment',
          query: {
            orderNumber: order.value.order.order_number,
          },
        });
        await router.push(ledgerDirectRoute);
        return;
      }
      order.value = await make();
      setCart(null);
